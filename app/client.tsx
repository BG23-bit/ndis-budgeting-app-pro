...
function getPublicHolidays(year: number, state: string): { date: string; name: string; dayOfWeek: number }[] {
  ...
  if (state === "ACT") {
    ...
    const dow = mar.getDay();
    const canberraDay = new Date(year, 2, 1 + ((8 - dow) % 7) + 7);
    addH(formatDate(canberraDay), "Canberra Day");
  }
  if (state === "VIC") {
    const jun = new Date(year, 5, 1);
    const dow = jun.getDay();
    const qb = new Date(year, 5, 1 + ((8 - dow) % 7) + 7);
    addH(formatDate(qb), "Queen's Birthday");
    const nov = new Date(year, 10, 1);
    const dow2 = nov.getDay();
    const mc = new Date(year, 10, 1 + ((9 - dow2) % 7));
    addH(formatDate(mc), "Melbourne Cup");
  }
  ...
  if (state === "QLD") {
    ...
    const dow = oct.getDay();
    const qb = new Date(year, 9, 1 + ((8 - dow) % 7) + 21);
    addH(formatDate(qb), "Queen's Birthday");
  }
  if (state === "WA") {
    const sep = new Date(year, 8, 1);
    const dow = sep.getDay();
    const qb = new Date(year, 8, 1 + ((8 - dow) % 7) + 21);
    addH(formatDate(qb), "Queen's Birthday");
  }
  if (state === "NT") {
    const may = new Date(year, 4, 1);
    const dow = may.getDay();
    const md = new Date(year, 4, 1 + ((8 - dow) % 7));
    addH(formatDate(md), "May Day");
    const jun = new Date(year, 5, 1);
    const dow2 = jun.getDay();
    const qb = new Date(year, 5, 1 + ((8 - dow2) % 7) + 7);
    addH(formatDate(qb), "Queen's Birthday");
  }
  if (state === "SA") {
    const oct = new Date(year, 9, 1);
    const dow = oct.getDay();
    const ld = new Date(year, 9, 1 + ((8 - dow) % 7));
    addH(formatDate(ld), "Labour Day");
  }
  if (state === "TAS") {
    const nov = new Date(year, 10, 1);
    const dow = nov.getDay();
    const rd = new Date(year, 10, 1 + ((8 - dow) % 7));
    addH(formatDate(rd), "Recreation Day");
  }
  ...
  return (
    <label className="flex items-center gap-3 cursor-pointer">
      <div onClick={() => props.onChange(!props.value)} style={{
        ...
        border: "1px solid " + (props.value ? "rgba(34,197,94,0.6)" : "rgba(239,68,68,0.5)"),
        transition: "all 0.2s",
      }}>
        <div style={{
          width: "20px", height: "20px", borderRadius: "50%", position: "absolute", top: "2px",
          left: props.value ? "25px" : "3px",
          background: props.value ? "#22c55e" : "#ef4444",
          transition: "all 0.2s",
        }} />
      </div>
      <div className="text-sm" style={{ color: "#b0a0d0" }}>
        {props.label}: <span style={{ color: props.value ? "#22c55e" : "#ef4444", fontWeight: "600" }}>
          {props.value ? "YES" : "NO"}
        </span>
      </div>
    </label>
...
function getSuggestions(line: any, rates: Rates) {
  ...
  if (line.hrsSun > 0) {
    const s = (rates.sun - rates.weekdayOrd) / divisor;
    const h = Math.min(line.hrsSun, Math.ceil(overBy / (s * 52)));
    suggestions.push("Reduce " + h + " Sunday hr" + (h > 1 ? "s" : "") + "/wk - saves " + money(s * h * 52) + "/yr");
  }
  if (line.hrsSat > 0) {
    const s = (rates.sat - rates.weekdayOrd) / divisor;
    const h = Math.min(line.hrsSat, Math.ceil(overBy / (s * 52)));
    suggestions.push("Reduce " + h + " Saturday hr" + (h > 1 ? "s" : "") + "/wk - saves " + money(s * h * 52) + "/yr");
  }
...
function calcPHImpact(line: SupportLine, holidays: { date: string; name: string; dayOfWeek: number }[], rates: Rates) {
  ...
  for (const h of holidays) {
    const dow = h.dayOfWeek;
    let normalRate = 0;
    let normalHrs = 0;

    if (dow >= 1 && dow <= 5) {
      normalRate = rates.weekdayOrd / divisor;
      normalHrs = line.hrsWeekdayOrd;
    } else if (dow === 6) {
      normalRate = rates.sat / divisor;
      normalHrs = line.hrsSat;
    } else {
      normalRate = rates.sun / divisor;
      normalHrs = line.hrsSun;
    }
    ...
    if (line.workOnPH) {
      ...
      extraCost += extra;
      details.push({ name: h.name, date: h.date, day: getDayName(dow), impact: extra, type: "extra" });
    } else {
      const saved = normalRate * normalHrs;
      savedCost += saved;
      details.push({ name: h.name, date: h.date, day: getDayName(dow), impact: saved, type: "saved" });
    }
  }
...
export default function PageClient() {
  const STORAGE_KEY = "ndis_budget_calc_pro_v5";
  const unlocked = true;
  ...
  function addLine() {
    setLines((prev) => [...prev, {
      id: uid(), code: "NEW", description: "New line", totalFunding: 0, ratio: "1:1", workOnPH: true,
  ...
  function exportCSV() {
    const header = ["Code","Description","Ratio","Work on PH","Total Funding","Weekly Total","PH Adjustment","Plan Total","Remaining"];
              ...
              <div className="grid grid-cols-1 gap-1 sm:grid-cols-2 lg:grid-cols-3">
                {holidays.map((h, i) => (
                  <div key={i} className="text-sm py-1 px-2 rounded" style={{ background: "rgba(255,255,255,0.03)" }}>
                    ...
                    <span style={{ color: "#8080a0" }}>({getDayName(h.dayOfWeek)})</span>{" "}
        ...
        <div className="rounded-2xl p-6 mb-6" style={{
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="grid gap-2">
              ...
              <div>PH adjustment: <span className="font-semibold" style={{ color: totals.totalPHAdjust > 0 ? "#ef4444" : "#22c55e" }}>
                {totals.totalPHAdjust > 0 ? "+" : ""}{money(totals.totalPHAdjust)}
              </span></div>
          ...
          <div className="mt-4">
            <div className="flex justify-between text-xs mb-1" style={{ color: "#b0a0d0" }}>
              <span>Used: {money(totals.planCost)}</span>
              <span>Budget: {money(totals.totalFunding)}</span>
            </div>
            <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: "8px", height: "12px", overflow: "hidden" }}>
              <div style={{
                ...
                height: "100%",
                borderRadius: "8px",
                background: totals.planCost > totals.totalFunding
                  ? "linear-gradient(90deg, #ef4444, #dc2626)"
                  : totals.planCost > totals.totalFunding * 0.9
                  ? "linear-gradient(90deg, #f59e0b, #d97706)"
                  : "linear-gradient(90deg, #22c55e, #16a34a)",
                transition: "width 0.3s",
              }} />
            </div>
            <div className="text-xs mt-1 text-right" style={{ color: "#b0a0d0" }}>
              {totals.totalFunding > 0 ? ((totals.planCost / totals.totalFunding) * 100).toFixed(1) : 0}% used
            </div>
          </div>
                ...
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                  <div className="rounded-xl p-4" style={{ background: "rgba(15,10,48,0.6)", border: "1px solid rgba(212,168,67,0.1)" }}>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      ...
                      <ToggleField label="Work on Public Holidays" value={l.workOnPH}
                        onChange={(v) => updateLine(l.id, { workOnPH: v })} />
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl p-4" style={{ background: "rgba(15,10,48,0.6)", border: "1px solid rgba(212,168,67,0.1)" }}>
                    ...
                    <div className="mt-4 text-sm" style={{ color: "#b0a0d0" }}>
                      ...
                      <div>PH adjustment: <span className="font-semibold" style={{ color: l.phAdjustment > 0 ? "#ef4444" : "#22c55e" }}>
                        {l.phAdjustment > 0 ? "+" : ""}{money(l.phAdjustment)}
                      </span></div>
                    ...
                    <div className="grid grid-cols-1 gap-1 sm:grid-cols-2">
                      {l.phImpact.details.map((d: any, i: number) => (
                        <div key={i} className="text-sm py-1" style={{ color: "#b0a0d0" }}>
                          {d.date} ({d.day}) — {d.name}: <span style={{ color: l.workOnPH ? "#ef4444" : "#22c55e", fontWeight: "600" }}>
                            {l.workOnPH ? "+" : "-"}{money(d.impact)}
                          </span>
                        </div>
                      ))}
                    </div>
