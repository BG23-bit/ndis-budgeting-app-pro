...
type ProviderDetails = { orgName: string; abn: string; contactName: string; email: string; phone: string; address: string; registrationNumber: string; defaultRates?: Partial<Rates>; customHolidays?: CustomHoliday[]; logo?: string };
...
<div class="footer">
  <div>${escapeHtml(providerDetails.orgName||"Kevria Calc")}</div>
...
<div className="text-sm font-semibold" style={{color:"#d4a843"}}>Weekly Roster{lineMode==="weekday"&&<span style={{color:"#64748b",fontWeight:"normal",fontSize:"0.8rem",marginLeft:"8px"}}>Weekdays only</span>}</div>
{(()=>{
const dh=rosterDays.filter((d:string)=>l.roster[d]?.enabled&&((l.roster[d].hours||0)+(l.roster[d].nightHours||0))>0);
...
<div style={{background:"#f8fafc",border:"1px solid rgba(212,168,67,0.3)",borderRadius:"16px",maxWidth:"680px",width:"100%",maxHeight:"90vh",overflowY:"auto",padding:"32px"}}>
  ...
  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-6">
    ...
    <div className="sm:col-span-2">
      ...
      <input ref={logoFileRef} type="file" accept="image/png,image/jpeg,image/webp" style={{display:"none"}} onChange={e=>{const f=e.target.files?.[0];if(f)handleLogoUpload(f);e.target.value="";}}/>
      {providerDetails.logo?(
        <div className="flex items-center gap-3 flex-wrap">
          <img src={providerDetails.logo} alt="Your logo" style={{maxHeight:"44px",maxWidth:"200px",background:"#ffffff",border:"1px solid #e2e8f0",borderRadius:"8px",padding:"5px 10px"}}/>
          <button onClick={()=>logoFileRef.current?.click()} className="kv-btn" style={{background:"rgba(212,168,67,0.1)",border:"1px solid rgba(212,168,67,0.3)",color:"#b8901a",padding:"7px 14px",borderRadius:"8px",cursor:"pointer",fontSize:"0.8rem",fontWeight:600}}>Replace</button>
